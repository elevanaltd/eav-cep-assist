...
CREATE OR REPLACE FUNCTION "public"."comments_broadcast_trigger"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  -- NEW and OLD are trigger variables, NOT tables - do not quote them
  -- They must be accessed WITHOUT quotes to work in trigger context
...
COMMENT ON FUNCTION "public"."comments_broadcast_trigger"() IS 'Production fix 2025-10-25: Corrected NEW/OLD variable access in trigger.
   
   ISSUE: Quoted NEW/OLD ("NEW", "OLD") treated as table references → missing FROM-clause error
   FIX: Unquoted NEW/OLD are trigger variables accessible in trigger context
   
   Trigger variables do NOT need quoting - they are special PostgreSQL variables
   available in trigger functions that reference the row being modified.
   
   Authority: error-architect (BLOCKING for production)
   Validation: Direct INSERT test must succeed without errors';
...
CREATE OR REPLACE FUNCTION "public"."user_has_script_access"("p_user_id" "uuid", "p_script_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_temp'
...
COMMENT ON CONSTRAINT "check_position_range" ON "public"."comments" IS 'Allows zero-length anchors (end_position = start_position) for script-level comments. Application validation (requireAnchors capability) controls whether zero-length anchors are allowed per app.';
...
CREATE TABLE IF NOT EXISTS "public"."dropdown_options" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "dropdown_options_field_name_check" CHECK (("field_name" = ANY (ARRAY['shot_type'::"text", 'location_start_point'::"text", 'movement_type'::"text", 'subject'::"text"])))
);
...
COMMENT ON COLUMN "public"."shots"."shot_type" IS 'Dropdown from dropdown_options: WS, MID, CU, FP, OBJ-L, OBJ-R, UNDER,MONTAGE,ESTAB,TRACK (fixed list, no other)';
...
COMMENT ON COLUMN "public"."shots"."movement_type" IS 'Type of camera movement (Tracking, Establishing, Standard, Photos). Determines which additional fields are required.';
...
CREATE TABLE IF NOT EXISTS "public"."user_profiles" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "user_profiles_role_check" CHECK (("role" = ANY (ARRAY['admin'::"text", 'client'::"text", 'employee'::"text", 'editor'::"text"])))
);
...
COMMENT ON CONSTRAINT "user_profiles_role_check" ON "public"."user_profiles" IS 'Allowed roles: admin, client, employee, editor. Editor role added 2025-11-07 for future edit-web app. Current apps (Library Manager, Script Builder) use employee role for script editing workflows.';
...
CREATE INDEX "idx_comments_resolved_by" ON "public"."comments" USING "btree" ("resolved_by") WHERE ("resolved_by" IS NOT NULL);



COMMENT ON INDEX "public"."idx_comments_resolved_by" IS 'Performance: Supports JOINs to user_profiles and comment resolution queries. Partial index (WHERE resolved_by IS NOT NULL) reduces index size.';
...
COMMENT ON INDEX "public"."idx_comments_user_id" IS 'Performance: Critical index for user comment lookups, RLS checks, and DELETE cascades. High-frequency JOIN path.';
...
CREATE INDEX "idx_shots_script_component_id" ON "public"."shots" USING "btree" ("script_component_id");
...
CREATE INDEX "idx_user_clients_granted_by" ON "public"."user_clients" USING "btree" ("granted_by") WHERE ("granted_by" IS NOT NULL);



COMMENT ON INDEX "public"."idx_user_clients_granted_by" IS 'Performance: Supports admin queries for grant history. Partial index (WHERE granted_by IS NOT NULL) reduces index size.';
...
ALTER TABLE ONLY "public"."shots"
    ADD CONSTRAINT "shots_script_component_id_fkey" FOREIGN KEY ("script_component_id") REFERENCES "public"."script_components"("id") ON DELETE CASCADE;
...
CREATE POLICY "employee_all_dropdown_options" ON "public"."dropdown_options" USING ((EXISTS ( SELECT 1
   FROM "public"."user_profiles"
...
COMMENT ON POLICY "employee_all_dropdown_options" ON "public"."dropdown_options" IS 'Employees have full access to dropdown_options for shot planning workflow';
...
CREATE POLICY "realtime_select_simple" ON "public"."scripts" FOR SELECT TO "authenticated" USING ("public"."user_has_script_access"("auth"."uid"(), "id"));
...
GRANT ALL ON FUNCTION "public"."user_has_script_access"("p_user_id" "uuid", "p_script_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."user_has_script_access"("p_user_id" "uuid", "p_script_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."user_has_script_access"("p_user_id" "uuid", "p_script_id" "uuid") TO "service_role";
