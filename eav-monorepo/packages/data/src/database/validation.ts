 ...
 *
 * Note: DOMPurify removed from pure data layer (browser-only dependency)
 * Sanitization should be done at presentation layer (UI components)
 */
...
export const scriptContentSchema = z.string()
  ...
  // Note: Sanitization removed from pure data layer
  // HTML sanitization should happen at UI rendering layer (browser-only)
  // Data layer only validates structure and format
