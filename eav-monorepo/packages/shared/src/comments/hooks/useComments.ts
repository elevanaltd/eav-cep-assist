...
import { useCommentsQuery } from '../state/useCommentsQuery'
import { useCommentMutations } from '../state/useCommentMutations'
import { useCommentStore } from '../state/commentStore'
import { getUserFriendlyErrorMessage } from '../../errors/errorHandling'
import { Logger } from '../../services/logger'
import type { CommentCapabilities } from '../domain/capabilities'
 ...
 */
export const useComments = (editor: Editor | null, scriptId: string | null, capabilities: CommentCapabilities) => {
  const commentsQuery = useCommentsQuery(scriptId)
  const mutations = useCommentMutations(capabilities)
  ...
  const loadCommentHighlights = useCallback(async (scriptId: string) => {
    ...
    try {
      ...
      const { getComments } = await import('../domain/repository')
      const { getSupabaseClient } = await import('../../lib/client/singleton')
  ...
  useEffect(() => {
    ...
    const handleBlur = () => {
      ...
      blurTimeoutId = setTimeout(() => {
        if (!isMountedRef.current || !scriptId) return
        ...
        loadCommentHighlights(scriptId)
      }, 500) // 500ms debounce
    }
  ...
  }, [editor, scriptId, loadCommentHighlights])
