    ...
    return {
      addCommentHighlight:
        ...
        ({ commands: _commands, state, dispatch }) => {
          ...
          const commentHighlightMark = state.schema.marks.commentHighlight;
          if (!commentHighlightMark) {
            console.warn('Comment highlight mark not found in schema');
            return false;
          }
