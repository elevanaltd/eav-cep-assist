...
describe('useCommentsQuery - Integration Tests (Testguard-Approved)', () => {
  ...
  it('does not fetch when no script selected', () => {
    const { result } = renderHook(() => useCommentsQuery(null), {
  ...
  it('uses 30-second stale time for comment freshness (architecture compliance)', () => {
    ...
    const { result } = renderHook(() => useCommentsQuery('script-123'), {
      wrapper: createTestWrapper(),
    })
