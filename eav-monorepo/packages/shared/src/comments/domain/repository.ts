...
import { batchRecoverCommentPositions } from './positionRecovery.js';
...
export async function createComment(
  ...
  try {
    // Validate input data
    const validationError = validateCommentData(data, capabilities);
    if (validationError) {
      return {
        success: false,
        error: {
        ...
        if (failed.length > 0) {
          console.error(`Position recovery: ${failed.length}/${positionsToUpdate.length} updates failed`,
            failed.map(({ result, index }) => ({
              commentId: positionsToUpdate[index]!.id, // Non-null assertion - index comes from positionsToUpdate array
...
function validateCommentData(data: CreateCommentData, capabilities: CommentCapabilities): string | null {
  ...
  // Capability-based validation: only enforce non-zero-length when requireAnchors=true
  if (capabilities.requireAnchors && data.endPosition <= data.startPosition) {
    return 'Text selection required for comments. Zero-length anchors not allowed in strict mode.';
  }
