...
import {
  ...
  cleanupTestData,
  authDelay,
} from '../../test/supabase-test-client';
...
import * as commentsLib from './repository';

// Helper function to get current user ID from authenticated client
...
async function ensureTestDataExists(client: SupabaseClient<Database>) {
  ...
  await client
    ...
    .upsert({
      ...
      client_filter: 'CLIENT_ALPHA',  // Matches client.test@example.com assignment for RLS testing
...
describe('Comments Infrastructure - Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    try {
      await signInAsTestUser(testSupabase, 'admin');
      if (TEST_SCRIPT_ID) {
        await testSupabase.from('comments').delete().eq('script_id', TEST_SCRIPT_ID);
      }
    } catch {
  ...
  describe('Comments Table Schema', () => {
    test('admin should create comment with required fields', async () => {
      await signInAsTestUser(testSupabase, 'admin');
      const adminUserId = await getCurrentUserId(testSupabase);
    ...
    test('admin should create threaded comment reply', async () => {
      ...
      const { data: parentComment, error: parentError } = await testSupabase
        .from('comments')
        .insert({
          script_id: TEST_SCRIPT_ID,
          user_id: adminUserId,
      ...
      const { data: replyComment, error: replyError } = await testSupabase
        .from('comments')
        .insert({
          script_id: TEST_SCRIPT_ID,
          user_id: adminUserId,
    ...
    test('admin should resolve comment with resolved_at and resolved_by', async () => {
      ...
      // Create comment
      const { data: comment, error: createError } = await testSupabase
        .from('comments')
        .insert({
          script_id: TEST_SCRIPT_ID,
          user_id: adminUserId,
      ...
      // Resolve comment
      const { data: resolvedComment, error: resolveError } = await testSupabase
        .from('comments')
        .update({
  ...
  describe('Comments RLS Security - CONTRACT-DRIVEN-CORRECTION', () => {
    test('admin should have full access to all comments', async () => {
      ...
      const { error: createError } = await testSupabase
        .from('comments')
        .insert({
          script_id: TEST_SCRIPT_ID,
          user_id: adminUserId,
    ...
    test('client user CANNOT create comments (read-only enforcement)', async () => {
      await authDelay();
      await signInAsTestUser(testSupabase, 'client');
      const clientUserId = await getCurrentUserId(testSupabase);

      // RLS policy correctly blocks client INSERT operations
      // Clients are read-only for comments (North Star I2: Multi-client data isolation)
      ...
      // RLS policy correctly blocks client creation
    ...
    test('unauthorized user should NOT see any comments', async () => {
      ...
      await authDelay();
      await signInAsTestUser(testSupabase, 'unauthorized');
      const { data, error } = await testSupabase
        .from('comments')
        .select('*')
        .eq('script_id', TEST_SCRIPT_ID);
    ...
    test('unauthorized user should NOT create comments', async () => {
      await authDelay();
      await signInAsTestUser(testSupabase, 'unauthorized');
      const unauthorizedUserId = await getCurrentUserId(testSupabase);
  ...
  describe('Comments Position Validation', () => {
    ...
    test('should validate start_position <= end_position', async () => {
      // DATABASE CONSTRAINT: CHECK (end_position >= start_position)
      // Migration: 20251102143000_fix_comments_position_constraint.sql
      //
      // Allows zero-length anchors (end_position = start_position) for cam-op-pwa
      // Still rejects inverted ranges (end_position < start_position)
      //
      // This test validates that inverted ranges are rejected at database level
    ...
    test('should allow zero-length anchors at database level', async () => {
      // DATABASE CONSTRAINT: CHECK (end_position >= start_position)
      // Migration: 20251102143000_fix_comments_position_constraint.sql
      //
      // Zero-length anchors (end_position = start_position) are allowed at DB level
      // Application-level validation (requireAnchors capability) controls whether
      // they're acceptable for each app:
      // - scripts-web: requireAnchors=true → enforces non-zero length
      // - cam-op-pwa: requireAnchors=false → allows zero-length
      //
      // This test validates that the database constraint permits zero-length anchors
      ...
      const { data, error } = await testSupabase
        ...
        .insert({
          script_id: TEST_SCRIPT_ID,
          user_id: adminUserId,
          content: 'Zero-length anchor comment',
          start_position: 0,
          end_position: 0 // Zero-length anchor (valid at DB level)
        })
        .select()
        .single();

      // Should succeed - database constraint allows zero-length anchors
      expect(error).toBeNull();
      expect(data).toBeDefined();
      expect(data?.start_position).toBe(0);
      expect(data?.end_position).toBe(0);
  ...
  describe('Comments Threading Behavior', () => {
    test('admin should cascade delete child comments when parent deleted', async () => {
      ...
      const { data: deletedReply, error: checkError } = await testSupabase
        .from('comments')
        .select('*')
  ...
  describe('Comments Performance Indexes', () => {
    ...
    test('admin should efficiently filter by resolved status', async () => {
      ...
      const { data: unresolvedComments, error } = await testSupabase
        .from('comments')
        .select('*')
        .eq('script_id', TEST_SCRIPT_ID)
        .is('resolved_at', null);
  ...
  describe('getComments Performance - N+1 Query Fix', () => {
    test('should fetch all user profiles in single query, not N+1 queries', async () => {
      ...
      const originalFrom = testSupabase.from.bind(testSupabase);
      const queryLog: string[] = [];

      // Test spy requires any types for runtime table interception
      /* eslint-disable @typescript-eslint/no-explicit-any */
      testSupabase.from = (((table: any) => {
        queryLog.push(String(table));
        return originalFrom(table);
      }) as any) as typeof testSupabase.from;
      /* eslint-enable @typescript-eslint/no-explicit-any */

      try {
        // This should fail - current implementation does N+1 queries
        const result = await commentsLib.getComments(testSupabase, TEST_SCRIPT_ID);
...
describe('Comments CRUD Functions - TDD Phase', () => {
  ...
  describe('createComment Function - TDD (WILL FAIL)', () => {
    test('should create comment and return CommentWithUser type', async () => {
      ...
      // Basic capabilities for CRUD testing
      const capabilities = {
      ...
      const result = await commentsLib.createComment(testSupabase, commentData, adminUserId, capabilities);
    ...
    test('should validate required fields and return error', async () => {
      ...
      const result = await commentsLib.createComment(testSupabase, invalidData, adminUserId, capabilities);
  ...
  describe('getComments Function - TDD (WILL FAIL)', () => {
    ...
    test('should filter comments by resolved status', async () => {
      ...
      // This will fail - function doesn't exist
      const result = await commentsLib.getComments(testSupabase, TEST_SCRIPT_ID, { resolved: false });
  ...
  describe('updateComment Function', () => {
    test('should update comment content and return updated comment', async () => {
      ...
      await signInAsTestUser(testSupabase, 'admin');
      const adminUserId = await getCurrentUserId(testSupabase);
      const { data: comment } = await testSupabase.from('comments').insert({
        script_id: TEST_SCRIPT_ID,
        user_id: adminUserId,
    ...
    test('should resolve comment with timestamp and user', async () => {
      ...
      const result = await commentsLib.resolveComment(testSupabase, comment!.id, adminUserId);
  ...
  describe('deleteComment Function - TDD (WILL FAIL)', () => {
    test('should soft delete comment (mark as deleted)', async () => {
      ...
      // This will fail - function doesn't exist
      const result = await commentsLib.deleteComment(testSupabase, comment!.id, adminUserId);

      expect(result.success).toBe(true);

      // Verify it's soft deleted (marked as deleted, not removed)
      const { data: deletedComment } = await testSupabase
        .from('comments')
        .select('*')
        .eq('id', comment!.id)
        .single();
  ...
  describe('POSITION DRIFT PERSISTENCE - TDD (RED PHASE)', () => {
    test('should persist highlightedText when creating comment', async () => {
      ...
      const { data: dbComment } = await testSupabase
        .from('comments')
  ...
  describe('CASCADE SOFT DELETE - TDD (WILL FAIL)', () => {
    test('should cascade delete parent with one child', async () => {
      ...
      // Create parent comment
      const { data: parent } = await testSupabase.from('comments').insert({
        script_id: TEST_SCRIPT_ID,
        user_id: adminUserId,
      ...
      const { data: child } = await testSupabase.from('comments').insert({
        script_id: TEST_SCRIPT_ID,
        user_id: adminUserId,
      ...
      const result = await commentsLib.deleteComment(testSupabase, parent!.id, adminUserId);
      expect(result.success).toBe(true);

      // Verify both parent and child are soft deleted
      const { data: deletedParent } = await testSupabase
        .from('comments').select('*').eq('id', parent!.id).single();
      const { data: deletedChild } = await testSupabase
    ...
    test('should cascade delete nested descendants (3+ levels)', async () => {
      ...
      const { data: grandchild } = await testSupabase.from('comments').insert({
        script_id: TEST_SCRIPT_ID,
        user_id: adminUserId,
        content: 'Parent comment',
        start_position: 8,
      ...
      const { data: greatGrandchild } = await testSupabase.from('comments').insert({
        script_id: TEST_SCRIPT_ID,
        user_id: adminUserId,
        content: 'Child comment',
        start_position: 12,
    ...
    test('should only delete target comment when no children exist', async () => {
      ...
      const { data: unrelated } = await testSupabase.from('comments').insert({
        script_id: TEST_SCRIPT_ID,
        user_id: adminUserId,
      ...
      const { data: unrelatedComment } = await testSupabase
    ...
    test('should handle deep nesting (5+ levels) efficiently', async () => {
      ...
      for (let level = 0; level < 6; level++) {
        const response = await testSupabase
          .from('comments')
          .insert({
            script_id: TEST_SCRIPT_ID,
            user_id: adminUserId,
      ...
      const startTime = Date.now();
      const result = await commentsLib.deleteComment(testSupabase, comments[0].id, adminUserId);
      const endTime = Date.now();
