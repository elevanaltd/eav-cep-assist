...
import { getSupabaseClient } from '../../lib/client/singleton';
...
export type { ComponentData } from '../../database/validation';
...
export async function saveScriptWithComponents(
  ...
  try {
    ...
    // Cross-environment base64 encoding for yjs_state (Uint8Array)
    // Node.js: Buffer.from() / Browser: btoa()
    /* eslint-disable no-undef -- Buffer/btoa are platform-specific globals (Node.js/browser) */
    const base64YjsState = yjsState
      ? (typeof Buffer !== 'undefined'
          ? Buffer.from(yjsState).toString('base64')
          : btoa(String.fromCharCode(...yjsState)))
      : '';
    ...
    if (!updatedScript) {
      throw new ScriptServiceError('Script not found after update', 'NOT_FOUND');
    }
