...
import { CommentPositionTracker } from '@workspace/shared/comments/extensions';
...
import { loadScriptForVideo, ComponentData, ScriptWorkflowStatus, generateContentHash } from '@workspace/shared/scripts';
import { Logger } from '@workspace/shared/services';
import { extractComponents as extractComponentsFromDoc } from '@workspace/shared/editor';
...
export const TipTapEditor: React.FC = () => {
  ...
  const editor = useEditor({
    ...
    extensions: [
      ...
      CommentHighlightExtension.configure({
        ...
        onHighlightHover: (commentId: string, _commentNumber: number, isHovering: boolean) => {
  ...
  useEffect(() => {
    ...
    if (!isEditable && lockStatus === 'unlocked') {
      ...
      showError('⚠️ You lost the edit lock. Refresh the page to edit again.');
    }
