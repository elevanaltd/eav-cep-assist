...
import * as useScriptLockModule from '@workspace/shared/editor'
...
describe('ScriptLockIndicator', () => {
  ...
  it('shows locked status with other user name', () => {
    ...
    // Verify locked status message with user name and instructions
    expect(screen.getByText(/Locked by Jane Smith/i)).toBeInTheDocument()
    expect(screen.getByText(/Contact them via Teams or refresh to check status/i)).toBeInTheDocument()

    // Verify yellow indicator
    const indicator = screen.getByRole('status')
    expect(indicator).toHaveClass('status-locked') // Yellow styling

    // Verify no action button (text message only)
  ...
  it('shows unlocked/lost status with red warning', () => {
    ...
    // Verify unlocked warning message with instructions
    expect(screen.getByText(/Lock released/i)).toBeInTheDocument()
    expect(screen.getByText(/Refresh page to edit again/i)).toBeInTheDocument()
