...
// RLS security tests now have complete test infrastructure
//
// **Infrastructure Complete**:
//   ✅ Test users created via Supabase Auth API (scripts/create-test-users-via-api.mjs)
//   ✅ user_clients junction table populated with CLIENT_ALPHA and CLIENT_BETA assignments
//   ✅ Test projects in seed.sql with matching client_filter values
//   ✅ client2 user (test-client2@another.com) for multi-client scenarios
//
// **Test Requirements**:
//   - Valid Supabase credentials in environment variables
//   - Run: node scripts/create-test-users-via-api.mjs before running tests
//   - Database migrated with RLS policies active
//
// **Test Coverage**: Comprehensive two-role security model (admin/client) validation
const skipIfNoEnv = describe; // Tests enabled - infrastructure complete
...
// Test data - Use client_filters matching seed.sql projects
const TEST_CLIENT_FILTER = 'CLIENT_ALPHA';  // Matches Test Project Alpha in seed.sql
const TEST_CLIENT_FILTER_2 = 'CLIENT_BETA';  // Matches Test Project Beta in seed.sql
