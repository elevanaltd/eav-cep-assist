...
describe('Supabase client', () => {
  ...
  it('should use singleton from @workspace/shared', async () => {
    // Critical test: Verify we're using the shared singleton, not creating a new instance
    // This prevents the "Multiple GoTrueClient instances" warning in production
    const { supabase: instance1 } = await import('./supabase')
    const { getSupabaseClient } = await import('@workspace/shared/client')
    const instance2 = getSupabaseClient()

    // Both should reference the SAME singleton instance
    expect(instance1).toBe(instance2)
    expect(instance1.auth).toBe(instance2.auth)
  })
})